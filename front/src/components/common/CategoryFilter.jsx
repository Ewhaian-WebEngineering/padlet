...
export default function CategoryFilter({
  ...
  return (
    <S.Container onMouseEnter={onMouseEnter}
      ...
      {visible && position.width > 0 &&
        createPortal(
          
          <S.PortalDropdown style={{ top: position.top, left: position.left, width: position.width }}>
            {categories.map((item, idx) => (
              <li key={idx} onClick={(e) => { e.stopPropagation(); onSelect(item); }}>
                {item}
              </li>
            ))}
          </S.PortalDropdown>
